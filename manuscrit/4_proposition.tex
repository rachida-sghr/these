...
\label{sec:executeea}
    ...
    Nous recourons à la méta-modélisation pour unifier les différentes activités d'architecture —~documentation, analyse et conception~— en les inscrivant dans le cadre méthodologique et technologique offert par l'IDM.
...
aux limitations des modèles contemplatifs qui font légion en EA. C'est donc pour l'analyse que les modèles
exécutables sont le plus valorisés et indispensables.

Ensuite, comme l'illustre la figure~\ref{fig:activite_ea} l'analyse joue un rôle centrale dans une démarche d'EA.
Le recours les modèles d'analyse peuvent directement servir à documenter l'EA. Il s'agit de modèles qui recourent
directement aux concepts du domaine de l'EA et qui ne nécessitent donc pas de changer de niveau d'abstraction.
...
même modèles. Dès lors que ces modèles sont validés à l'issue de les activités d'analyse et de conception,
il devient possible de les utiliser à pour la documentation (en mettant
automatiquement à jour les anciens modèles d'architecture) et pour l'implémentation (voir figure~\ref{fig:activite_ea}. L'implémentation peut 
alors se faire par transformation de modèles, en transformant les modèles d'architecture vers la plate-forme cible.
...
L'analyse de la structure repose essentiellement sur la formalisation du métamodèle EA2M et a pour
objectif l'intégration des modèles provenant des différentes vues d'architecture pour garantir la cohérence
globale de l'architecture. L'analyse du comportement de l'architecture repose quant à elle
essentiellement sur la simulation des modèles issus de l'analyse de la structure.
Nous le détaillons les activités l'analyse de la structure et du comportement dans la section~\ref{sec:analyse}.

L'approche que nous proposons est illustrée par la figure~\ref{fig:approche_conceptuelle}. Nous proposons un
processus de modélisation et d'analyse précis en identifiant quatre rôles participant à ce processus~: l'architecte métier,
...
métier. L'architecte fonctionnel traduit la vue métier en vue fonctionnelle 
...
métier. Le rôle de l'architecte d'entreprise consiste alors à orchestrer ces
collaborations.
...
les architectes des différentes vue. L'architecte d'entreprise est alors
...
L'approche proposée s'inscrit en outre dans un cadre d'architecture qui se repose sur le métamodèle
EA2M. Ce cadre est illustré par la figure~\ref{fig:vue_integration}.
...
% intervenant à plusieurs niveaux de l'entreprise~: \item un architecte d'entreprise dont la mission est de
% mettre en cohérence l'ensemble des vues.
