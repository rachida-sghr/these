...
Dans la partie précédente nous avons présenté le domaine de l'EA en nous focalisons
...
mises en œuvre pour rendre les modèles utilisés sur tout le cycle de vie d'un système logiciel productifs, notamment
la méta-modélisation et les transformations de modèle.

Dans ce chapitre, nous examinons tout d'abord les pratiques actuellles d'EA, dans la section~\ref{sec:bilan_ea},
en nous appuyant sur une étude de la doucmentation de projets dédiés aux Smart Grids
corroborant la revue de la littéreture présentée dans la première partie de ce mémoire.
...
baptisant EAT-ME (section~\ref{sec:EAT-ME}). Ce métamodèle est la pierre angulaire du \emph{framework} ExecuteEA que nous
...
par l'emploi et la production d'artefacts de représentations informels et purement
descriptifs.
...
amenés à faire le parallèle entre les deux domaines et
à envisager l'IDM comme cadre méthodologique et technologique pour l'EA.
Dès lors, nous justifions le recours à une démarche IDM  pour traiter les
...
\subsection{L'Ingénierie Dirigée par les Modèles~:\\ un cadre méthodologique et technologique}
    ...
    confrontées aux limites de ces artefacts hétérogènes et de nature informelle (mise à
    ...
    Le recours intensif aux modèles exécutables dans les activités d'EA, et
    pour chacune des vues d'architecture, permet de créer une architecture d'entreprise
    souple et réactive à l'évolution de la stratégie d'entreprise. Les modèles d'une vue donnée peuvent être automatiquement
...
Tout d'abord, nous avons étudié les spécifications des démonstrateurs orientés Smart Grids
européens et en particulier celles de ADDRESS et PREMIO. Le choix de ces deux
démonstrateurs est justifié par la participation du département MIRE à ces deux
...
enjeux des Smart Grids et à l'exploration des pratiques de spécification de ce type
de grands projets impliquant plusieurs partenaires. Les deux spécifications se
...
imposer un canevas pour la spécification de cas d'utilisation pour les Smart Grids puis à
...
\subsection{Identification des concepts}
    ...
    Ensuite, l’objectif de EAT-ME est de permettre de vérifier la cohérence internes à une vue mais
...
métamodèle.
Dans le cadre de EAT-ME, ces contraintes sont exprimés à travers les liens
...
La définition du métamodèle EAT-ME a nécessité une analyse du domaine de l'EA en générale, et dans le
contexte des Smart Grids en particulier, conformément à la démarche préconisée par l'IDM.  

Dans le chapitre suivant, nus décrivons une implémentation du \emph{framework} ExecuteEA que nous
éprouvons au cas métier d'une gestion de flotte de véhicules électriques.
